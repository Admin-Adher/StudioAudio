...
- `packages\StudioAudio-Windows-x64-Portable-0.2.0.zip`
...
```bash
bash scripts/build_desktop_macos.sh 0.2.0 arm64
```

Le script utilise un environnement `.venv-desktop-macos-arm64`, génère le véritable
`Studio Audio.app`, puis l'unique livrable
`packages/StudioAudio-macOS-arm64-0.2.0.dmg`. Le workflow macOS actuel cible
